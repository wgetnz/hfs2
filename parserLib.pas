...
const
  ...
  while i <= l do
    begin
    if strAt(txt, MARKER_QUOTE, i) then
      begin
      inc(quoteDepth);
      inc(i, length(MARKER_QUOTE));
      continue;
      end;
    if strAt(txt, MARKER_UNQUOTE, i) then
      begin
      if quoteDepth > 0 then dec(quoteDepth);
      inc(i, length(MARKER_UNQUOTE));
      continue;
      end;
    if (quoteDepth = 0) and (txt[i] = '%') then
      begin
      b:=i;
      e:=b+1;
      if e > l then break;
      ...
      if not charInSet(txt[e], ['_','a'..'z','A'..'Z']) then
        begin
        inc(i);
        continue;
        end;
      ...
      if (e <= l) and (txt[e] = '%') then
        ...
        if s <> newS then
          begin
          idsStack[recurLevel]:=s;
          try applyMacrosAndSymbols2(newS, cb, cbData, idsStack, recurLevel) except end;
          idsStack[recurLevel]:='';
